...
const Home = () => {
    ...
    const validateHandler = (event) => {
        ...
        if(event.target.value != null){
            ...
            if(regExp.test(event.target.value)){
                axios.post("http://localhost:3002/api/insert/user/verify/email", {email: userName, verifycode: event.target.value})
                .then(res => { 
                    if(res.status === 200){
    ...
    const validateCaptcha = (value) => {
        ...
        if(value != null){
            //post data
            axios.post("http://localhost:3002/api/insert/user", { email: userName })
            .then(res =>{
                if(res.status === 200){
            ...
            axios.post("http://localhost:3002/api/insert/user/verify", { email: userName })
                    .then(res =>{
                        if(res.status === 200){
